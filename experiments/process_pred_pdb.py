...
parser.add_argument(
    ...
    type=str,
    default='/home/psh/protein-frame-flow/pred_samples')
parser.add_argument(
    '--num_processes',
...
parser.add_argument(
    ...
    type=str,
    default='/home/psh/protein-frame-flow/pred_pkls')
parser.add_argument(
    '--debug',
...
def main(args):
    ...
    for pdb in os.listdir(pred_dir):
        pdb_dir = os.path.join(pred_dir, pdb)
        all_file_paths = [
            os.path.join(pdb_dir, x)
            for x in os.listdir(pdb_dir) if '.pdb' in x]
        total_num_paths = len(all_file_paths)
        pdb_write_dir = os.path.join(write_dir, pdb)
        ...
        metadata_path = os.path.join(pdb_write_dir, metadata_file_name)
...
if __name__ == "__main__":
    ...
    print(f"args.write_dir: '{args.write_dir}'")
    main(args)