...
def get_capri(model_pdb, ref_pdb, receptor_chain, ligand_chain):
    ...
    for ch, idxs in ref_coord_all.items():
        for idx in idxs:
            if idx not in model_coord_all[ch].keys():
                continue
    ...
    model_idx_interface, model_pair_interface = get_interface_idx(model_coord_all_overlap, model_idx_tot, receptor_chain, ligand_chain, iface_cutoff = 5.0)
...
if __name__ == '__main__':
    ...
    dirname= "/home/psh/protein-frame-flow/inference_outputs/CDRFlow_v2.4.0_loop_ppi/2026-01-29_00-29-37/epoch=57-step=41470/benchmark_wo_perturb_w_ag_cond"
    pdbs = [
        p for p in glob(f"{dirname}/*/*.pdb")
        if not p.endswith("_trans.pdb")
    ]
    ...
    for i in tqdm(pdbs):
        ...
        basename = '_'.join([pdb_id, hchain, lchain, agchain])
        # ref_file = f"/home/kkh517/benchmark_set_after210930_ag/{basename}/new.pdb"
        ref_file = f"/home/psh/benchmark_after210930/pdb_unnum/{basename}.pdb"
        ...
        try:
            ...
            info_dict['name'].append(pdbname)
            info_dict['capri'].append(None)
            info_dict['dockq'].append(None)
            info_dict['lrmsd'].append(None)
            info_dict['irmsd'].append(None)
            info_dict['fnat'].append(None)
